...
class GreenhousePanel extends LitElement {
      ...
      <div class="card">
        ...
        >
          <button
            ...
            @click=${() => this._switchLightMode("growth")}
          >
            🌞 Growth Mode
          </button>
          <button
            class="button ${this._lightMode === "rest" ? "" : "secondary"}"
            @click=${() => this._switchLightMode("rest")}
          >
            🌙 Rest Mode
          </button>
        </div>
    ...
    return html`
      <div class="card wide-card">
        ...
        <div class="device-list">
          ...
          <div class="device-card" @click=${this._handleDeviceClick}>
            <div class="device-icon">🌡️</div>
            <div class="device-name">Temp Sensor</div>
            <div class="device-status">${this._simTemp}°C</div>
          </div>

          <div class="device-card" @click=${this._handleDeviceClick}>
            <div class="device-icon">💧</div>
            <div class="device-name">Humidity Sensor</div>
            <div class="device-status">${this._simHumidity}%</div>
          </div>

          <div class="device-card" @click=${this._handleDeviceClick}>
            <div class="device-icon">☀️</div>
            <div class="device-name">Light Sensor</div>
            <div class="device-status">${this._simLight} lux</div>
          </div>
  ...
  _renderLightingSchedule() {
    return html`
      <div class="card">
        ...
        <button class="button secondary">📝 Custom Scene</button>
      </div>
    `;
  }
  ...
  _switchLightMode(mode) {
    this._lightMode = mode;

    // Update light color temperature based on mode
    if (mode === "growth") {
      // Warm light for growth (3000K)
      ...
      // Cool light for rest (5000K) - or turn off
      this.hass.callService("light", "turn_on", {
        entity_id: "light.grow_light",
        color_temp: 200, // 5000K in mireds
        brightness: 128,
      });
    }
